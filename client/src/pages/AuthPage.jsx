      ...
      <section className="flex min-h-screen flex-col px-6 py-6 md:px-12 lg:px-16">
        ...
        <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center py-12">
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            {!isLogin && <div><label className="label" htmlFor="auth-name">Full name</label><input id="auth-name" className="input" name="name" value={form.name} onChange={update} placeholder="Alex Morgan" required /></div>}
            <div><label className="label" htmlFor="auth-email">Email address</label><input id="auth-email" className="input" type="email" name="email" value={form.email} onChange={update} placeholder="you@example.com" required /></div>
            <div><label className="label" htmlFor="auth-password">Password</label><div className="relative"><input id="auth-password" className="input pr-11" type={show ? "text" : "password"} name="password" value={form.password} onChange={update} placeholder="8–72 characters" minLength={8} maxLength={72} required /><button type="button" aria-label={show ? "Hide password" : "Show password"} onClick={() => setShow(!show)} className="absolute right-3 top-3 text-slate-400">{show ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></div>
            {!isLogin && <><div><label className="label" htmlFor="auth-target-company">Target company</label><select id="auth-target-company" className="input" name="targetCompany" value={form.targetCompany} onChange={update}>{["Google", "Amazon", "Microsoft", "Meta", "Other"].map((item) => <option key={item}>{item}</option>)}</select></div><div><label className="label" htmlFor="auth-coding-goal">Coding goal</label><input id="auth-coding-goal" className="input" name="codingGoal" value={form.codingGoal} onChange={update} /></div></>}
