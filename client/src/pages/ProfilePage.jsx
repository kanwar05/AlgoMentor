...
export default function ProfilePage() {
  ...
  const submit = async (event) => {
    ...
    const next = demoMode ? form : (await api.put("/auth/me", form)).data.user;
    updateUser(next); setSaved(true); setTimeout(() => setSaved(false), 2000);
  };
  return (
    <>
      ...
      <form onSubmit={submit} className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
        <div className="card space-y-5">
          <div className="flex items-center gap-4 border-b pb-5"><span className="grid h-16 w-16 place-items-center rounded-2xl bg-violet-500 font-display text-2xl font-extrabold text-white">{user.name[0]}</span><div><p className="font-display text-xl font-extrabold">{user.name}</p><p className="mt-1 flex items-center gap-1.5 text-sm text-slate-400"><Mail size={14} /> {user.email}</p></div></div>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><div><label className="label">Target company</label><div className="relative"><Building2 className="absolute left-3 top-3 text-slate-400" size={18}/><select className="input pl-10" name="targetCompany" value={form.targetCompany} onChange={update}>{["Google", "Amazon", "Microsoft", "Meta", "Other"].map((item) => <option key={item}>{item}</option>)}</select></div></div><div><label className="label">Problems per week</label><input type="number" min="1" max="50" className="input" name="weeklyGoal" value={form.weeklyGoal} onChange={update} /></div></div>
          <button className="btn-primary">{saved ? <><Check size={17} /> Saved</> : <><Save size={17} /> Save profile</>}</button>
        </div>
