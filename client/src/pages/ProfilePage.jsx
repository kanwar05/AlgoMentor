    ...
    <>
      ...
      <form onSubmit={submit} className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
        <div className="card space-y-5">
          ...
          <div><label className="label" htmlFor="profile-name">Full name</label><div className="relative"><UserRound className="absolute left-3 top-3 text-slate-400" size={18}/><input id="profile-name" className="input pl-10" name="name" value={form?.name || ""} onChange={update} /></div></div>
          <div><label className="label" htmlFor="profile-coding-goal">Coding goal</label><div className="relative"><Flag className="absolute left-3 top-3 text-slate-400" size={18}/><input id="profile-coding-goal" className="input pl-10" name="codingGoal" value={form?.codingGoal || ""} onChange={update} /></div></div>
          <div className="grid gap-4 sm:grid-cols-2"><div><label className="label" htmlFor="profile-target-company">Target company</label><div className="relative"><Building2 className="absolute left-3 top-3 text-slate-400" size={18}/><select id="profile-target-company" className="input pl-10" name="targetCompany" value={form?.targetCompany || "Other"} onChange={update}>{["Google", "Amazon", "Microsoft", "Meta", "Other"].map((item) => <option key={item}>{item}</option>)}</select></div></div><div><label className="label" htmlFor="profile-weekly-goal">Problems per week</label><input id="profile-weekly-goal" type="number" min="1" max="50" className="input" name="weeklyGoal" value={form?.weeklyGoal || 1} onChange={update} /></div></div>
