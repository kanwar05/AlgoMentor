import { ArrowDown, ArrowUp, ArrowUpRight, Bookmark, Building2, Check, Filter, Sparkles, Target, X } from "lucide-react";
...
export default function RecommendationsPage() {
  ...
  const recommendations = (data?.recommendations || []).filter((problem) => !hidden.has(problem.problemId));

  const submitFeedback = async (problem, feedback) => {
    const problemId = problem?.problemId;
    if (!problemId || saving.has(problemId)) return;
    setFeedbackError("");
    setSaving((items) => new Set(items).add(problemId));
    try {
      if (!demoMode) {
        await api.put(`/recommendations/${encodeURIComponent(problemId)}/feedback`, { feedback });
        await refetch();
      } else if (feedback !== "save_for_later") {
        setHidden((items) => new Set(items).add(problemId));
      }
    } catch (requestError) {
      setFeedbackError(requestError.response?.data?.message || requestError.message || "Could not save recommendation feedback");
    } finally {
      setSaving((items) => {
  ...
  const feedbackActions = [
    ["too_easy", "Too easy", ArrowUp],
    ["too_hard", "Too hard", ArrowDown],
    ["already_solved", "Already solved", Check],
    ["not_relevant", "Not relevant", X],
    ["save_for_later", "Save for later", Bookmark]
  ];
  ...
  return (
    <>
      ...
      {feedbackError && <p className="card mb-4 border-rose-200 text-sm text-rose-500">{feedbackError}</p>}
      {recommendations.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {recommendations.map((problem, index) => {
          const isSaving = saving.has(problem?.problemId);
          return <article key={problem?.problemId || problem?.title || index} className="card group flex flex-col hover:-translate-y-1 hover:shadow-soft"><div className="flex items-start justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 font-display text-sm font-extrabold text-slate-500 dark:bg-white/10">{String(index + 1).padStart(2, "0")}</span><div className="flex items-center gap-2">{problem?.savedForLater && <span className="rounded-full bg-violet-100 px-2 py-1 text-[10px] font-bold text-violet-700 dark:bg-violet-400/10 dark:text-violet-300">Saved</span>}<StatusPill>{problem?.difficulty || "Unknown"}</StatusPill></div></div><h2 className="mt-5 font-display text-lg font-extrabold leading-snug">{problem?.title || "Untitled problem"}</h2><p className="mt-1 text-xs text-slate-400">{problem?.platform || "Unknown platform"}</p><div className="mt-4 flex flex-wrap gap-1.5">{(problem?.topics || []).map((topic) => <StatusPill key={topic}>{topic}</StatusPill>)}</div><div className="mt-5 flex items-start gap-2 rounded-xl bg-violet-50 p-3 text-xs leading-5 text-violet-700 dark:bg-violet-400/10 dark:text-violet-300"><Sparkles size={15} className="mt-0.5 shrink-0" />{problem?.reason || "Recommended from your current practice profile."}</div><div className="mt-4 flex flex-wrap gap-1.5 border-t pt-4">{feedbackActions.map(([feedback, label, Icon]) => <button key={feedback} type="button" onClick={() => submitFeedback(problem, feedback)} disabled={isSaving || (problem?.savedForLater && feedback === "save_for_later")} className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-1.5 text-[10px] font-bold text-slate-600 hover:bg-violet-100 hover:text-violet-700 disabled:opacity-50 dark:bg-white/[.06] dark:text-slate-300"><Icon size={11} />{label}</button>)}</div><div className="mt-auto flex items-center justify-between pt-5"><span className="text-xs font-semibold text-slate-400">{problem?.companies?.slice(0, 2).join(" · ")}</span>{problem?.link && <a href={problem.link} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-sm font-bold text-violet-500">Solve now <ArrowUpRight size={16} /></a>}</div></article>;
        })}
