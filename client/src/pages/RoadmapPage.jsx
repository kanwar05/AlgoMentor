...
export default function RoadmapPage() {
  const { data, loading } = useRemoteData("/roadmap", demoRoadmap);
  if (loading) return <LoadingState />;
  return (
    <>
      <PageHeader eyebrow="Dependency-aware learning" title="Your DSA roadmap" description="Prerequisites come first. Weak topics become focus nodes. Advanced patterns follow when the foundation is ready." />
      <div className="grid gap-4 xl:grid-cols-[1.4fr_.6fr]">
          ...
          <div className="relative mx-auto max-w-2xl">
            {data.path.map((step, index) => (
              <div key={step.topic}>
                ...
                {index < data.path.length - 1 && <div className="grid h-10 place-items-center"><ArrowDown size={18} className="text-slate-300" /></div>}
              </div>
            ))}
          </div>
        </div>
        <div className="space-y-4">
          ...
          <div className="card"><p className="font-display text-lg font-bold">Current focus</p><div className="mt-4 space-y-2">{data.focusTopics.length ? data.focusTopics.map((topic, index) => <div key={topic} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 dark:bg-white/[.04]"><span className="grid h-8 w-8 place-items-center rounded-lg bg-rose-100 text-xs font-extrabold text-rose-600">{index + 1}</span><span className="text-sm font-bold">{topic}</span></div>) : <p className="text-sm text-slate-500">Keep logging problems to personalize your focus.</p>}</div></div>
